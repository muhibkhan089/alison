...
class Dob extends AbstractWidget
{
    ...
    {
        if($this->viewHelper->isSubaccountLoggedIn() == 1 && $this->moduleConfig->isSharedSessionEnabled()) {
            $subaccountData = $this->customerSession->getSubaccountData();
            if($subaccountData) {
                $userDob = $subaccountData->getDob();    
            }
        } else {
            $userDob = $date;
        }

        $this->setTime($userDob ? strtotime($userDob) : false);
        $this->setValue($this->applyOutputFilter($userDob));

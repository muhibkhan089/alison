...
class ProductsGrid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem(
            'save',
            [
                'label' => __('Add to Whishlist'),
                'url' => $this->getUrl('*/*/addPost', ['wishlist_id' => $this->getRequest()->getParam('id')]),
                'confirm' => __('Are you sure you want to add this product(s) to whishlist?')
            ]
        );
    ...
    {
        return $this->getUrl('wishlist/manage/add/id/' . $this->getRequest()->getParam('id'));
    }
    ...
    {
        $wishlistId = $this->getRequest()->getParam('id') ?: 0;
        $addButton = $this->getLayout()->createBlock('Magento\Backend\Block\Widget\Button')
            ->setData(array(
            'label'     => 'Back to Wishlist',
            'onclick'   => 'setLocation("' . $this->getUrl('wishlist/manage/view', ['id' => $wishlistId]) . '")',
            'class'   => 'task'
        ))->toHtml();
        return $addButton.parent::getMainButtonsHtml();
    }
}
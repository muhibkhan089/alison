<?php
namespace Redgiant\Dailydeals\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function isDealProduct($productId)
    {
        ...
        $dailydealcollection->addFieldToFilter('rg_product_sku', ['eq'=>$sku]);
    ...
    public function getbundleProductDiscount($sku)
    {
        ...
        if ($dailydealcollection->getFirstItem()->getRgDiscountType() ==1) {
            return '<div style=" margin-top:20px; "><strong>Save:'.$this->getcurrencySymbol().''.number_format($dailydealcollection->getFirstItem()->getRgDiscountAmount(), 2).'</strong></div>';
        } elseif ($dailydealcollection->getFirstItem()->getRgDiscountType() ==2) {
            return '<div style="margin-top:20px;"><strong>OFF:'.number_format($dailydealcollection->getFirstItem()->getRgDiscountAmount(), 2).'%</strong></div>';
        }
    }
    ...
    public function getDealProductDiscountValue($dealproductsku)
    {
        $dailydealcollection=$this->getDailydealcollection();
        $dailydealcollection->addFieldToSelect('*');
        $dailydealcollection->addFieldToFilter('rg_product_sku', ['eq'=>$dealproductsku]);
        
        return $dailydealcollection->getFirstItem()->getRgDiscountAmount();
    }
    ...
    public function getDealProductPrice($dealproductsku)
    {
        ...
        return $dailydealcollection->getFirstItem()->getRgProductPrice();
    }
    ...
    public function getDailydealToDate($dealproductsku)
    {
        ...
        return $dailydealcollection->getFirstItem()->getRgDateTo();
    }
    ...
    public function getDailydealFromDate($dealproductsku)
    {
        ...
        return $dailydealcollection->getFirstItem()->getRgDateFrom();
    }
    ...
    public function getDealOffValue($dealproductsku)
    {
        ...
        $discountType=$dailydealcollection->getFirstItem()->getRgDiscountType();
        if ($discountType ==1) {

<?php
namespace Redgiant\Dailydeals\Observer;
...
class DealProductDiscountPrice implements ObserverInterface
{
    ...
    public function __construct(
        ...
        \Redgiant\Dailydeals\Helper\Data $helper,
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $dailydealcollection->addFieldToFilter('rg_product_sku', ['eq' => $product->getSku()]);

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $objDate =$objectManager ->create('Magento\Framework\Stdlib\DateTime\DateTime');
       
        $curdate=strtotime($objDate->gmtDate("Y-m-d H:i:s"));
        $Todate=strtotime($dailydealcollection->getFirstItem()->getRgDateTo());
        $fromdate=strtotime($dailydealcollection->getFirstItem()->getRgDateFrom());
        
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        
        $configPath = "rg_dailydeal/general/dailydeal_enabled";
            ...
            if ($item->getProduct()->getTypeId() == "bundle") {
                ...
                foreach ($item->getQuote()->getAllItems() as $bundleitems) {
                    ...
                    if (max($bundleItemPrice) == $bundleitems->getProduct()->getPrice()) {
                        if ($dailydealcollection->getFirstItem()->getRgDiscountType() == 1) {
                            $finalprice=$bundleitems->getProduct()->getFinalPrice()-$dailydealcollection->getFirstItem()->getRgDiscountAmount();
                        } elseif ($dailydealcollection->getFirstItem()->getRgDiscountType() == 2) {
                            $finalprice=$bundleitems->getProduct()->getFinalPrice()-(($item->getProduct()->getFinalPrice()*$dailydealcollection->getFirstItem()->getRgDiscountAmount())/100);
                        }
                ...
                if (isset($groupedProductFalg)) {
                    ...
                    foreach ($item->getQuote()->getAllItems() as $groupedItem) {
                        if (min($groupedItemPrice) == $groupedItem->getProduct()->getPrice()) {
                            if ($dailydealcollection->getFirstItem()->getRgDiscountType() == 1) {
                                $price=$groupedItem->getProduct()->getPrice()-$dailydealcollection->getFirstItem()->getRgDiscountAmount();
                            } elseif ($dailydealcollection->getFirstItem()->getRgDiscountType() == 2) {
                                $price=$groupedItem->getProduct()->getPrice()-(($groupedItem->getProduct()->getPrice()*$dailydealcollection->getFirstItem()->getRgDiscountAmount())/100);
                            }
                            break;
                        }
                    }
                } else {
                    $price = $dailydealcollection->getFirstItem()->getRgProductPrice();
                }
